...
public final class Encoder {
	...
	public static AztecCode encode(byte[] data, int minECCPercent, int userSpecifiedLayers, Charset charset) {
		...
		if (userSpecifiedLayers != DEFAULT_AZTEC_LAYERS) {
			...
			encodeRefactor3(eccBits, compact, wordSize, stuffedBits, usableBitsInLayers);
		} else {
			wordSize = 0;
			stuffedBits = null;
		...
		int matrixSize;
		matrixSize = encodeRefactor2(compact, baseMatrixSize, alignmentMap);
		BitMatrix matrix = new BitMatrix(matrixSize);

		// draw data bits
		encodeRefactor5(compact, layers, messageBits, baseMatrixSize, alignmentMap, matrix);

		// draw mode message
		drawModeMessage(matrix, compact, matrixSize, modeMessage);

		// draw alignment marks
		encoderefactor6(compact, baseMatrixSize, matrixSize, matrix);
	...
	private static void encoderefactor6(boolean compact, int baseMatrixSize, int matrixSize, BitMatrix matrix) {
		if (compact) {
	...
	private static void encodeRefactor5(boolean compact, int layers, BitArray messageBits, int baseMatrixSize,
			int[] alignmentMap, BitMatrix matrix) {
		for (int i = 0, rowOffset = 0; i < layers; i++) {
			int rowSize = (layers - i) * 4 + (compact ? 9 : 12);
			for (int j = 0; j < rowSize; j++) {
				int columnOffset = j * 2;
				for (int k = 0; k < 2; k++) {
					encodeRefactor(messageBits, baseMatrixSize, alignmentMap, matrix, i, rowOffset, rowSize, j, columnOffset, k);
				}
			}
			rowOffset += rowSize * 8;
		}
	...
	private static void encodeRefactor3(int eccBits, boolean compact, int wordSize, BitArray stuffedBits,
	...
	private static int encodeRefactor2(boolean compact, int baseMatrixSize, int[] alignmentMap) {
		int matrixSize;
		if (compact) {
	...
	private static void encodeRefactor(BitArray messageBits, int baseMatrixSize, int[] alignmentMap, BitMatrix matrix,
			int i, int rowOffset, int rowSize, int j, int columnOffset, int k) {
	...
	private static void drawModeMessage(BitMatrix matrix, boolean compact, int matrixSize, BitArray modeMessage) {
		int center = matrixSize / 2;
		if (compact) {
			drawModeMessageRefactor(matrix, modeMessage, center);
		} else {
			drawModeMessageRefactor2(matrix, modeMessage, center);
		}
	}

	private static void drawModeMessageRefactor2(BitMatrix matrix, BitArray modeMessage, int center) {
		for (int i = 0; i < 10; i++) {
	...
	private static void drawModeMessageRefactor(BitMatrix matrix, BitArray modeMessage, int center) {
		for (int i = 0; i < 7; i++) {
